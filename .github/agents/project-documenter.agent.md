...
- **Target language**: Write all generated documentation in German.
- **Umlauts are mandatory**: Use correct German characters (`ä`, `ö`, `ü`, `Ä`, `Ö`, `Ü`, `ß`) in prose. Do not replace them with transliterations like `ae`, `oe`, `ue`, or `ss`, except where identifiers, file names, or code require ASCII.
...
Language requirements for this step:
- The full document must be written in German.
- Use correct German umlauts and `ß` in natural-language text.
- Keep source-code identifiers, commands, file names, and paths unchanged.
...
- **Output location is fixed**: Generate documentation artifacts only under `docs/` (for example `docs/project-summary.md`, `docs/project-summary.docx`, `docs/diagrams/*`).
- **No project-root documents**: NEVER create `.docx`, `.doc`, `.rtf`, `.md`, or diagram files in the repository root.
